...
export default function PageTransition({ children, pathname }: PageTransitionProps) {
  // Different animation types we can switch between
  const animations = {
    fadeUp: {
  ...
  // Choose animation type based on path (could be randomized or customized)
  const getAnimationVariant = (path: string) => {
    // Simple hashing of path to choose animation
    const pathHash = path.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const animationKeys = Object.keys(animations) as Array<keyof typeof animations>;
